...
from .reference_angles import (
    SQUAT_KNEE_EXTENSION_STANDING_POSE_DEG,
    SQUAT_KNEE_PARALLEL_DEG,
    SQUAT_KNEE_SHALLOW_WARNING_DEG,
    SQUAT_KNEE_DEEP_WARNING_DEG,
    SQUAT_HYSTERESIS_BUFFER_DEG,
)
...
class SquatConfig(RepCounterConfig):
    ...
    1. Angle Thresholds: Define the knee angle ranges for different squat phases
       (see backend/counters/reference_angles.py for ACSM-aligned literature notes).
       - extension_threshold: Knee angle when standing (pose-estimation calibrated)
       - flexion_threshold: Knee angle at bottom (parallel squat ~90° per common ACSM-aligned coaching)
    ...
    """
    # ===== ANGLE THRESHOLDS (in degrees) — defaults from reference_angles.py =====
    extension_threshold: float = SQUAT_KNEE_EXTENSION_STANDING_POSE_DEG
    flexion_threshold: float = SQUAT_KNEE_PARALLEL_DEG
    
    # ===== HYSTERESIS BUFFER =====
    # Prevents false state transitions from pose estimation noise
    buffer: float = SQUAT_HYSTERESIS_BUFFER_DEG
    
    # ===== SMOOTHING SETTINGS =====
    angle_buffer_size: int = 3
    
    # ===== FEEDBACK THRESHOLDS =====
    shallow_warning_angle: float = SQUAT_KNEE_SHALLOW_WARNING_DEG
    deep_warning_angle: float = SQUAT_KNEE_DEEP_WARNING_DEG
