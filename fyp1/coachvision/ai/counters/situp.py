...
from .reference_angles import (
    SITUP_TORSO_UP_DEG,
    SITUP_TORSO_DOWN_DEG,
    SITUP_BUFFER_DEG,
    SITUP_KNEE_TARGET_DEG,
    SITUP_KNEE_TOLERANCE_DEG,
)
...
class SitUpConfig(RepCounterConfig):
    ...
    """
    # Torso vs horizontal — see reference_angles.py
    extension_threshold: float = SITUP_TORSO_UP_DEG
    flexion_threshold: float = SITUP_TORSO_DOWN_DEG
    
    buffer: float = SITUP_BUFFER_DEG
    
    # Smoothing settings
    angle_buffer_size: int = 3
    
    # Form thresholds
    knee_angle_target: float = SITUP_KNEE_TARGET_DEG
    knee_angle_tolerance: float = SITUP_KNEE_TOLERANCE_DEG
