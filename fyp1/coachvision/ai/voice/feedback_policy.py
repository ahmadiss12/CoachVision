...
from ..counters.reference_angles import (
...
class FeedbackPolicy:
    ...
    def decide(
        ...
        if "DOWN" in state_upper and not self._bottom_cue_spoken_this_rep:
            ...
            if angle < SQUAT_KNEE_DEEP_WARNING_DEG:
                self._bottom_cue_spoken_this_rep = True
                return FeedbackLabel.TOO_DEEP
            if SQUAT_KNEE_PARALLEL_DEG < angle < SQUAT_KNEE_SHALLOW_WARNING_DEG:
                self._bottom_cue_spoken_this_rep = True
                return FeedbackLabel.SHALLOW_DEPTH
            if SQUAT_KNEE_DEEP_WARNING_DEG <= angle < SQUAT_KNEE_PARALLEL_DEG:
                self._bottom_cue_spoken_this_rep = True
                return FeedbackLabel.GOOD_DEPTH
